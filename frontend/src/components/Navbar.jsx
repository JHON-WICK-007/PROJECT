  ...
  return (
    <motion.nav
      ...
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled
        ? "bg-black/20 backdrop-blur-md shadow-2xl shadow-black/30"
        : "bg-transparent"
        }`}
    >
          ...
          <div className="hidden lg:flex items-center space-x-2">
            {navLinks.map((link) => (
              <Link
                key={link.path}
                to={link.path}
                className={`relative px-5 py-2 font-medium transition-all duration-300 nav-link ${isActive(link.path)
                  ? "text-yellow-500"
            ...
            <div className="px-6 py-6 space-y-3">
              ...
              {navLinks.map((link) => (
                <Link
                  key={link.path}
                  to={link.path}
                  className={`block px-5 py-3 rounded-lg font-medium transition-all duration-300 ${isActive(link.path)
